...
bool MissionTest::start_cmd(const AP_Mission::Mission_Command& cmd)
{
    ...
    if (AP_Mission::is_nav_cmd(cmd)) {
        num_nav_cmd_runs = 0;
        hal.uartD->printf("started cmd #%d id:%d Nav\n",(int)cmd.index,(int)cmd.id);
    }else{
        num_do_cmd_runs = 0;
        hal.uartD->printf("started cmd #%d id:%d Do\n",(int)cmd.index,(int)cmd.id);
    }
...
bool MissionTest::verify_cmd(const AP_Mission::Mission_Command& cmd)
{
    if (AP_Mission::is_nav_cmd(cmd)) {
        ...
        if (num_nav_cmd_runs < verify_nav_cmd_iterations_to_complete) {
            hal.uartD->printf("verified cmd #%d id:%d Nav iteration:%d\n",(int)cmd.index,(int)cmd.id,(int)num_nav_cmd_runs);
            return false;
        }else{
            hal.uartD->printf("verified cmd #%d id:%d Nav complete!\n",(int)cmd.index,(int)cmd.id);
            return true;
        }
    }else{
        num_do_cmd_runs++;
        if (num_do_cmd_runs < verify_do_cmd_iterations_to_complete) {
            hal.uartD->printf("verified cmd #%d id:%d Do iteration:%d\n",(int)cmd.index,(int)cmd.id,(int)num_do_cmd_runs);
            return false;
        }else{
            hal.uartD->printf("verified cmd #%d id:%d Do complete!\n",(int)cmd.index,(int)cmd.id);
            return true;
        }
    }
}
...
void MissionTest::init_mission()
{
    ...
    if (!mission.add_cmd(cmd)) {
        hal.uartD->print("failed to add command\n");
    }
...
// print_mission - print out the entire mission to the uartD
void MissionTest::print_mission()
{
    ...
    if (mission.num_commands() == 0) {
        hal.uartD->printf("No Mission!\n");
        return;
    }

    hal.uartD->printf("Mission: %d commands\n",(int)mission.num_commands());
    ...
    for(uint16_t i=0; i<mission.num_commands(); i++) {
        ...
        hal.uartD->printf("Cmd#%d mav-id:%d ", (int)cmd.index, (int)cmd.id);

        // print whether nav or do command
        if (AP_Mission::is_nav_cmd(cmd)) {
            hal.uartD->printf("Nav ");
        }else{
            hal.uartD->printf("Do ");
        }
...
void MissionTest::run_resume_test()
{
    ...
    hal.uartD->printf("Stopping mission\n");
    mission.stop();
    ...
    hal.uartD->printf("Resume mission\n");
    mission.resume();
...
void MissionTest::run_set_current_cmd_test()
{
    ...
    hal.uartD->printf("Setting current command to 2\n");
...
void MissionTest::run_set_current_cmd_while_stopped_test()
{
    ...
    hal.uartD->printf("Setting current command to 5\n");
...
void MissionTest::run_replace_cmd_test()
{
    ...
    if (!mission.replace_cmd(4, cmd)) {
        hal.uartD->printf("failed to replace command 4\n");
    }else{
        hal.uartD->printf("replaced command #4 -------------\n");
        // print current mission
        print_mission();
    }
...
void MissionTest::run_max_cmd_test()
{
    ...
    for (i=0; i<num_commands; i++) {
        if (!mission.read_cmd_from_storage(i,cmd)) {
            hal.uartD->printf("failed to retrieve command #%u\n",(unsigned int)i);
            failed_to_read = true;
            break;
        }else{
            if (cmd.content.location.alt == i) {
                hal.uartD->printf("successfully read command #%u\n",(unsigned int)i);
            }else{
                hal.uartD->printf("cmd %u's alt does not match, expected %u but read %u\n",
...
{
    hal.uartD->println("AP_Mission library test\n");

    // display basic info about command sizes
    hal.uartD->printf("Max Num Commands: %d\n",(int)mission.num_commands_max());
    hal.uartD->printf("Command size: %d bytes\n",(int)AP_MISSION_EEPROM_COMMAND_SIZE);
}
